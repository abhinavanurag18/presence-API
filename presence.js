            ...
            if (message.type === 'utf8') {
                ...
                if (userId === false) {
					...
					if (findClient(rjson.networkId) != -1) {
						...
						connection.sendUTF(JSON.stringify({d:'d'}));
					}
                } else { 
					// Get message content
                    var rjson = JSON.parse(message.utf8Data);
                    ...
                    switch (rjson.type) {
						...
						{
							var groupId = rjson.group;
							var filepath = "/var/www/html/tbjs2/samples/card-39.tb";
							var buffer;
							fs.open(filepath, 'r', function(err, fd) {
							    fs.fstat(fd, function(err, stats) {
							        ...
							        var data = buffer.toString('utf-8', 0, bufferSize);
							        console.log(data);
							    	var message = { type: msgSendTurtleData, data: data };
									broadcastToGroup(groupId, message);
							        fs.close(fd);
							    });
							});
